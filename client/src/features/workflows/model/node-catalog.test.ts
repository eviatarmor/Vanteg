...
describe("workflow node catalog", () => {
  ...
  it("exposes Setup fields on HTTP Request and related HTTP nodes", () => {
    ...
    expect(http?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["method", "url", "query", "headers", "body", "timeout"])
    )
    ...
    expect(http?.fields.find((field) => field.key === "query")?.control).toBe("keyValue")
    expect(http?.fields.find((field) => field.key === "headers")?.control).toBe("keyValue")
    ...
    expect(poll?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["method", "url", "query", "headers", "interval"])
    )
    expect(poll?.fields.find((field) => field.key === "headers")?.control).toBe("keyValue")

    const download = getNodeType("http-download")
    expect(download?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["url", "headers", "path"])
    )
    expect(download?.fields.find((field) => field.key === "headers")?.control).toBe("keyValue")
  ...
  it("polishes email send/reply/forward fields and form trigger controls", () => {
    ...
    expect(send?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["to", "cc", "subject", "body"])
    )
    ...
    expect(reply?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["to", "cc", "subject", "body"])
    )
    ...
    expect(forward?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["to", "cc", "subject", "body"])
    )
    ...
    expect(formId?.control).toBe("resource")
  ...
  it("polishes If/Switch/Filter/Delay field controls", () => {
    ...
    expect(ifNode?.fields.map((field) => field.key)).toEqual(["conditions"])
    expect(ifNode?.fields.find((field) => field.key === "conditions")?.control).toBe("conditions")

    const filter = getNodeType("filter")
    expect(filter?.fields.map((field) => field.key)).toEqual(["conditions"])
    expect(filter?.fields.find((field) => field.key === "conditions")?.control).toBe("conditions")

    const sw = getNodeType("switch")
    expect(sw?.fields.map((field) => field.key)).toEqual(["expression", "routes"])
    expect(sw?.fields.find((field) => field.key === "expression")?.control).toBe("expression")
    expect(sw?.fields.find((field) => field.key === "routes")?.control).toBe("routes")

    const delay = getNodeType("delay")
    expect(delay?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["duration", "unit", "until"])
    )
  ...
  it("polishes Code / Transform / Set / Merge / Loop field controls", () => {
    ...
    expect(code?.fields.map((field) => field.key)).toEqual(
    ...
    expect(set?.fields.find((field) => field.key === "mapping")?.control).toBe("mapping")
    ...
    expect(merge?.fields.map((field) => field.key)).toEqual(
    ...
    expect(loop?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["items", "concurrency"])
    )
    expect(loop?.fields.find((field) => field.key === "items")?.control).toBe("expression")
    const concurrency = loop?.fields.find((field) => field.key === "concurrency")
    expect(concurrency?.control).toBe("number")
  ...
  it("exposes Setup fields on Schedule, Webhook, Poll URL, RSS, and Manual triggers", () => {
    ...
    expect(schedule?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["cron", "timezone", "intervalType"])
    )
    expect(schedule?.fields.find((field) => field.key === "cron")?.control).toBe("cron")
    ...
    expect(webhook?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["path", "method", "secret"])
    )
    ...
    expect(rss?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["url", "interval"])
    )
    ...
    expect(manual?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["examplePayload"])
    )
  ...
  it("polishes Notification / File / Respond-to-webhook field controls", () => {
    ...
    expect(notification?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["channel", "title", "message", "severity"])
    )
    ...
    expect(file?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["path", "operation", "content"])
    )
    ...
    expect(respond?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["status", "body", "headers"])
    )
    ...
    expect(respond?.fields.find((field) => field.key === "headers")?.control).toBe("keyValue")
  })
  ...
  it("exposes textarea/select/number Setup fields on AI, Classify, and Extract", () => {
    ...
    expect(getNodeType("ai")?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["prompt", "model", "temperature"])
    )
    expect(getNodeType("ai-classify")?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["prompt", "labels", "model", "temperature"])
    )
    expect(getNodeType("ai-extract")?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["prompt", "schema", "model", "temperature"])
    )
    expect(getNodeType("ai-extract")?.fields.find((field) => field.key === "schema")?.control).toBe(
      "schema"
    )
  })
  ...
  it("exposes operation select, query textarea, and table input on Database CRUD", () => {
    ...
    expect(database?.fields.map((field) => field.key)).toEqual(
      expect.arrayContaining(["operation", "table", "query"])
    )
