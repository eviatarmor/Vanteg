...
describe("NodePickerDialog", () => {
  it("lets the user pick a connector from the more dialog", async () => {
    ...
    render(
      <NodePickerDialog open onOpenChange={onOpenChange} onAdd={onAdd} />
    )
    ...
    expect(onAdd).toHaveBeenCalledWith(expect.objectContaining({ id: "github" }))
  ...
  it("lists extra slack triggers and actions after opening the app", async () => {
    ...
    expect(screen.getByRole("button", { name: /App mentioned/ })).toBeInTheDocument()
