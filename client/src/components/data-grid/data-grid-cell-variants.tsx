...
export function SecretCell<TData>({
  cell,
  tableMeta,
  rowIndex,
  columnId,
  ...
  const onChange = React.useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      setValue((current) => applySecretInput(current, event.target.value));
    },
    [],
  );
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        <span data-slot="grid-cell-content">{maskSecretLast(value)}</span>
      )}
    </DataGridCellWrapper>
  );
}
