...
function useTimePickerGroupState() {
  ...
  return { groupContextValue, getColumns };
}

function TimePickerPanel(props: DivProps) {
  const { className, ...panelProps } = props;
  const { groupContextValue } = useTimePickerGroupState();
  ...
  return (
    <TimePickerGroupContext.Provider value={groupContextValue}>
      <div
        ...
        {...panelProps}
        className={cn(
          "flex min-w-max flex-col items-stretch gap-0",
          className,
        )}
      />
...
function TimePickerContent(props: TimePickerContentProps) {
  ...
  const { groupContextValue, getColumns } = useTimePickerGroupState();
  ...
  return (
    <TimePickerGroupContext.Provider value={groupContextValue}>
      <PopoverContent
        ...
        {...contentProps}
        className={cn(
          "flex-row gap-0 p-0 w-auto min-w-max",
          className,
        )}
